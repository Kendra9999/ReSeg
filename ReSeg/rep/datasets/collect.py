...
def collate(batch):
    ...
    all_overlap_patches = []
    all_overlap_patches_labels = []
    all_overlap_patches_girds = []
    all_whole_images = []
    all_whole_images_labels = []

    for data in batch:
        if "view1_mesh" in data:
            r_batch["img_metas"].append({"style": "overlap"})

            all_overlap_patches.append(data["view1"])
            all_overlap_patches_labels.append(data["label"])
            all_overlap_patches_girds.append(data["view1_mesh"])
            all_overlap_patches.append(data["view2"])
            all_overlap_patches_labels.append(data["label"])
            all_overlap_patches_girds.append(data["view2_mesh"])
            
            all_overlap_patches.append(data["view1_p1"])
            all_overlap_patches_labels.append(data["view1_p1_label"])
            all_overlap_patches_girds.append(data["view1_p1_mesh"])
            all_overlap_patches.append(data["view1_p2"])
            all_overlap_patches_labels.append(data["view1_p2_label"])
            all_overlap_patches_girds.append(data["view1_p2_mesh"])
            all_overlap_patches.append(data["view2_p1"])
            all_overlap_patches_labels.append(data["view2_p1_label"])
            all_overlap_patches_girds.append(data["view2_p1_mesh"])
            all_overlap_patches.append(data["view2_p2"])
            all_overlap_patches_labels.append(data["view2_p2_label"])
    ...
    r_batch["img"]["overlap_patches_labels"] = torch.stack(all_overlap_patches_labels, dim=0)
