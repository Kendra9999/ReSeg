...
class ResNet3d(nn.Module):
    ...
    def _inflate_conv_params(conv3d, state_dict_2d, module_name_2d,
        ...
        if flag == 1:
            new_weight = conv2d_weight.data.unsqueeze(1).expand_as(
                conv3d.weight)
        else:
        ...
        for name, module in self.named_modules():
            if isinstance(module, ConvModule):
                ...
                if original_conv_name + '.weight' not in state_dict_r2d:
                    ...
                    if original_conv_name =='conv1':
                        # self._inflate_conv_params(module.conv, state_dict_r2d,
                        #                           original_conv_name,
                        #                           inflated_param_names,flag=1)
                        logger.info(f'Do not load weights for conv1.')
                    else:
    ...
    def _make_stem_layer(self):
        ...
        self.maxpool = nn.MaxPool3d(
            kernel_size=(3, 3, 3), #(1, 3, 3),
            stride=(self.pool1_stride_t, self.pool1_stride_s,
                    self.pool1_stride_s),
            padding=(1, 1, 1)) #(0, 1, 1))

        self.pool2 = nn.Identity() #nn.MaxPool3d(kernel_size=(2, 1, 1), stride=(2, 1, 1))
    ...
    def forward(self, x, return_first_emb=False):
        """Defines the computation performed at every call.
